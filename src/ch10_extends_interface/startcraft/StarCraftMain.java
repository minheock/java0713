...
public class StarCraftMain {
	public static void main(String[] args) {
		...
		// 형 변환이 되었기 때문에
		// 부모가 가지고 있지않은 메소드는 사용불가
		// highTemplar2.Storm();
		// startList.get(4).Storm(); //형 변환되어 안됨.
		// 부모 => 자식 형변환후 자식의 메소드 사용
		// 다운 캐스팅은 
		((HighTemplar)starList.get(4)).Storm();
		
		// 
		StarUnit scv = new StarUnit("SCV", 5, 60, 0);
		// 일반적으로 부모 객체는 자식 객체로 형 변환 할 수 없다ㅏ.
		// instanceof 형 변환 가능 체크 true/false
