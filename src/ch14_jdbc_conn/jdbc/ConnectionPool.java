...
// Connection 객체를 미리 생성해서 풀(pool)에 보관 관리하는 클래스
...
public class ConnectionPool {
	...
	private ConnectionPool() {
		try {
			initPool(); // 클래스를 인스터스화 할때 initPool()을 호출하여
						// 풀을 초기화
	...
	// 풀 내의 모든 데이터베이스 연결을 닫는 메서드(전체종료시)
	...
	private synchronized void initPool() throws SQLException {
		...
		for(int i = 0 ; i < maxConn; i++) {
			pool.add(factory.getConnection()); // 프로퍼티 maxConn수량만큼
		}
	}
