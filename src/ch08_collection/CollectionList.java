...
//		<> 이부분을 제네릭(Generic) 이라고 함.
//		코드에서 타입의 안정성을 보장 할 수 있으며 불 필요한 타입 캐스팅을 줄일 수 있음.
//		제네릭은 객체만(참조타입) 담을 수 있음.
//		Wrapper Class (기본타입을 객체로 만든 클래스)
		ArrayList<Integer> intList = new ArrayList<Integer>();
//		래퍼 클래스는 (byte, short, int, long, float, double, char, boolean)
//		의 기본 타입 이름의 앞 글자 대문자로 되어있음.
//		list 기본 메서드 .add(요소) 요소 추가 시 사용
		intList.add(5);
		intList.add(1);
		intList.add(2);
		intList.add(4);
...
//		.get(index) 해당 인덱스 값을 리턴.
		System.out.println(intList.get(0));
//		.set(index, value) 요소 변경
		students.set(0, "펭귄");
		System.out.println(students);
//		.contains(value) 리스트안에 해당 값이 존재하면 true 를 리턴, 아닐 경우 false
		System.out.println(students.contains("길동"));
//		.indexOf(value) 리스트 안에 해당 값이 몇 번에 존재하는지 리턴 없으면 -1.
		System.out.println(students.indexOf("팽수"));
//		.isEmpty() 리스트가 비어있는지 체크 (true/false)
		System.out.println(students.isEmpty());
//		.size() 리스트의 요소 수
		System.out.println(students.size());
//		.remove(value) 해당 값이 있으면 삭제 없으면 아무일도 안일어남.
		students.remove("홍길동");
		System.out.println(students);
//		.clear() 리스트 비움 (size 0)
		students.clear();
...
//		2 리스트 복사
		ArrayList<String> copList2 = new ArrayList<String>(students);
		System.out.println(copList2);
